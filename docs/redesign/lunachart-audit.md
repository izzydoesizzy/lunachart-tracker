...
`:1479-1482`. The dashed coverline is drawn at a constant `36.38` — correct only for the built-in sample cycle. Any real user sees a line that means nothing, while the legend asserts it is "drawn 0.05°C above highest of 6 pre-shift temps." A misplaced coverline on a real chart can directly mislead an avoidance user.
...
`◉ ▤ ✦ ❡ ★ ⬆ ⊘` render inconsistently across platforms and read as placeholder.
